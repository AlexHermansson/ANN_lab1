...
T_test = T[-test_size:]
T_train = T[:-test_size]
T_train+=np.random.normal(0,sigma, T_train.shape)
...
history = model.fit(X_train, T_train, batch_size=b_size, epochs=epochs, callbacks=[EarlyStopping(patience = 20)], verbose=0, validation_split=0.4)
...
y = model.predict(X_test, verbose=2, batch_size=b_size_test)
t = np.arange(y.size)

test_MSE = model.evaluate(X_test, T_test, verbose = 0, batch_size=b_size_test)
print('test error: ', test_MSE)

prediction=model.predict(X, verbose=2, batch_size=b_size_test)
